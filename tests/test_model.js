...
assert.deepStrictEqual(Array.from(shifted.rows, row => row.key), ["TAB", "B", "1", "code:20"])
assert.strictEqual(shifted.title, "SUPER + SHIFT")
assert.strictEqual(shifted.rows[0].label, "Tab")
assert.strictEqual(shifted.rows[3].label, "-")
...
assert.deepStrictEqual(Array.from(columns[0], row => row.key), ["TAB", "B"])
assert.deepStrictEqual(Array.from(columns[1], row => row.key), ["1", "code:20"])
...
assert.strictEqual(Model.keyLabel("code:20", 0), "-")
assert.strictEqual(Model.keyLabel("code:21", 0), "=")
assert.strictEqual(Model.keyLabel("code:113", 0), "←")
assert.strictEqual(Model.keyLabel("code:999", 0), "Key 999")
